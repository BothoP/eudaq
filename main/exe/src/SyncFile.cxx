...
int main(int argc, char ** argv) {
    ...
    // writer->SetFilePattern("/home/paschen/tbdata/modified/run002172.raw");
    // writer->SetFilePattern("/home/paschen/tbdata19/modified/run000622.raw");
    writer->SetFilePattern("/run/media/paschen/Data/TB19/modified/run002141.raw");
    ...
    while (reader.NextEvent()) {
        ...
        // std::cout << "Create new Detector Event for holding global event" << std::endl;
        ...
        // std::cout << "Done creating global event" << std::endl;
        // copy flags
        ...
        for(size_t i=0; i<n_sub; i++) {
            ...
            if(subevents[i].empty()) {
                ...
                if(typeIDs[i] != tlu_id && eudaq::PluginManager::GetTriggerID(*read_event->GetEventPtr(i)) > 32768) {
                    std::cout << eudaq::Event::id2str(typeIDs[i]) << " " << subtype[i] << " " << eudaq::PluginManager::GetTriggerID(*read_event->GetEventPtr(i)) << std::endl;
                    triggerIDs[i]++;
                    ...
                    triggerIDs[i] = triggerID_offsets[i] + eudaq::PluginManager::GetTriggerID(*read_event->GetEventPtr(i));
                    // check for overflow  TODO: define detection limit
                    if (triggerIDs[i] < std::max((long) triggerID_offsets[i] + 1, (long)triggerID_old - 32760) && triggerID_old - triggerIDs[i] > 32000) {
                        std::cout << "overflow detected " << subtype[i] << " new: " << triggerIDs[i] << " old: " << triggerID_old << std::endl;
        ...
        for (size_t i=0; i<n_sub; i++) {
            if ((typeIDs[i] != tlu_id) && (triggerIDs[i] != (events.front().GetEventNumber() + 1))) {
                std::cout << i << " " << eudaq::Event::id2str(typeIDs[i]) << ":" << subtype[i] << " " << triggerIDs[i] << std::endl;
                got_mismatch = true;
                new_mismatch = true;
            }
        ...
        if (got_mismatch) {
            ...
            for (size_t i=0; i<n_sub; i++) {
                ...
                // check other sub events
                    ...
                    std::cout << "delete " << i << " " << eudaq::Event::id2str(typeIDs[i]) << ":" << subtype[i] << " " << triggerIDs[i] << std::endl;
                    subevents[i].pop_front();
                    if (!subevents[i].empty()) {
                        triggerID_old = triggerIDs[i];
        ...
        // no mismatch: write out event
            ...
            // write out first event in deque and delete
            writer->WriteEvent(events.front());
            std::cout << "Delete global event " << events.front().GetEventNumber() << " after writing" << std::endl;
            events.pop_front();
        }
    }
    ...
    for (unsigned i=0; i < n_sub; i++) {
        ...
        std::cout << eudaq::Event::id2str(typeIDs[i]) << " " << subtype[i] << " " << subevents[i].size() << std::endl;
    }
